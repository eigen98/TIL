...
/*
 ...
 <p>, </p> 사이에는 main, div, p 태그를 제외한 다른 태그들이 존재할 수 있으며, 예시에서 <br>와 같이 여는 태그만 존재할 수 있고, 여는 태그와 닫는 태그가 올바른 쌍으로 존재한다. 이때, 올바른 쌍은 아직 닫히지 않는 태그가 있을 때 다른 닫는 태그가 올 수 없다. 예를 들어, <b>a<i></b></i>는 올바른 쌍이 아니고, <b>a<i>b</i></b>은 올바른 쌍이다.
...
func solution(){
    var input = readLine()!
    //input = input.replacingOccurrences(of: "<main>", with : "")
    parseHTML(input)
}
...
func parseHTML(_ html: String) {
    ...
    for div in divs {
        ...
        let paragraphs = div.components(separatedBy: "<p>")
